class TileLookupManager {
  ...
  getTilePath2D = (edges, threshold, tilesize) => {
    if (this.cachingEnabled) {
      ...
      if (this.cachedPaths[hash]) {
        ...
        const path2D = this._createTileSVGPath2D(edges, threshold, tilesize);
        this.cachedPaths[hash] = path2D;
        return path2D;
      }
    } else {
      return this._createTileSVGPath2D(edges, threshold, tilesize);
    }
  };
  ...
  _createTileSVGPath2D = (edges, threshold, tileSize) => {
    const pathRaw = this._lookupTilePathData(edges, threshold);
    let svgData = '';
    ...
    const path2D = new Path2D(svgData);
    return path2D;
  };
