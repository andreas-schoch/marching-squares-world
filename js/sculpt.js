class SculptComponent {
  ...
  sculpt = (evt) => {
    ...
    // console.log('-----shift pressed', this.worldRef.input._mappings.shift.active);

    if (this._lastSculpt) {
      const changeSinceLast = util.vector.distance([x, y], this._lastSculpt.position);
      if (changeSinceLast < this.sleepDistance && this._lastSculpt.direction === direction) {
        console.log('sleep');
        return;
      }
    }
    ...
    for (let offsetY = -this.radiusXY; offsetY <= this.radiusXY; offsetY++) {
      for (let offsetX = -this.radiusXY; offsetX <= this.radiusXY; offsetX++) {
        ...
        // if (currentDensity >= this.worldRef.tileDensityMax + 3 && !evt.shiftKey) {
